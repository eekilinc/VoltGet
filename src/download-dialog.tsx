...
function detectSiteProfile(url: string, pageUrl?: string, filename?: string): SiteProfile {
  ...
  if (
    /\.(mp3|wav|flac|m4a|aac|ogg)($|\?)/i.test(combined) ||
    combined.includes('soundcloud.com') ||
    combined.includes('spotify.com') ||
    combined.includes('bandcamp.com')
  ) {
    return 'audio';
  }
  ...
  if (
    /tiktok\.com|twitter\.com|x\.com|instagram\.com|facebook\.com|reddit\.com|vimeo\.com|dailymotion\.com|twitch\.tv/i.test(
      combined
    )
  ) {
    return 'social';
  ...
  if (
    ...
    combined.includes('master.txt') ||
    combined.includes('/hls/') ||
    combined.includes('cdnimages') ||
    combined.includes('playmix') ||
    combined.includes('molystream') ||
    combined.includes('ddizi') ||
    combined.includes('hdfilmcehennemi') ||
    combined.includes('dizibox') ||
    combined.includes('yabancidizi') ||
    combined.includes('sinefy') ||
    combined.includes('sezonlukdizi')
  ) {
    return 'hls';
  }
...
function extractCleanTitle(d: any): string {
  ...
  try {
    ...
    if (
      clean &&
      clean.length > 3 &&
      !/^(master|video|playlist|sublist|index|watch)$/i.test(clean)
    ) {
      return clean.charAt(0).toUpperCase() + clean.slice(1);
    }
    if (segments.length > 0) {
      const prevSeg = decodeURIComponent(segments.pop() || '')
        .replace(/[-_]+/g, ' ')
        .trim();
...
function DialogApp() {
  ...
  const applyData = (d: any) => {
    ...
    if (profile === 'file') {
      ...
      const hlsFormats = [
        {
          id: 'best',
          resolution: '🎬 Tam Film / Dizi Videosu',
          ext: 'mp4',
          note: 'Orijinal Video (En Yüksek Çözünürlük)',
        },
        {
          id: 'bestaudio/best',
          resolution: '🎵 Sadece Ses Parçası',
          ext: 'mp3',
          note: 'MP3 Ses Kaydı',
        },
      ];
  ...
  const processParsedFormats = (allFormats: any[], vFormats?: any[]) => {
    ...
    const validVideo = (
      vFormats || allFormats.filter((f: any) => !f.isAudioOnly && f.height)
    ...
    const heights = Array.from(new Set(validVideo.map((f: any) => f.height as number))).sort(
      ...
      case 'file':
        return {
          ...
          ext:
            '.' +
            (data?.url
              ? data.url.split('/').pop()?.split('?')[0]?.split('.').pop() || 'bin'
              : 'bin'),
  ...
  return (
    <div
      style={{
        ...
        boxShadow:
          '0 24px 60px rgba(0, 0, 0, 0.95), 0 0 0 1px rgba(255, 255, 255, 0.08), inset 0 1px 0 rgba(255, 255, 255, 0.1)',
        height: '100vh',
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 6, ...noDragRegion }}>
          <button
        ...
        {siteProfile === 'hls' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                >
                  <div
                    style={{
                      fontWeight: 800,
                      fontSize: 12,
                      color: !isAudioMode ? '#60a5fa' : '#f8fafc',
                    }}
                  >
            ...
            <div
              onClick={() => {
                setIsAudioMode(true);
                setSelectedFormat('bestaudio/best');
              }}
              style={{
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                >
                  <div
                    style={{
                      fontWeight: 800,
                      fontSize: 12,
                      color: isAudioMode ? '#fbbf24' : '#f8fafc',
                    }}
                  >
                    Sadece Ses İzi (MP3)
                  </div>
                ...
                <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 2 }}>
                  Yalnızca Türkçe dublaj / orijinal ses parçası yüksek kalitede MP3 olarak
                  ayıklanır.
                </div>
              </div>
            </div>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 6 }}>
                  ...
                  <button
                    ...
                    style={{
                      background:
                        selectedFormat === (videoFormats[0]?.id || 'best') ||
                        selectedFormat === 'best'
                          ? 'rgba(37, 99, 235, 0.35)'
                          : 'rgba(30, 41, 59, 0.6)',
                      border:
                      ...
                      <button
                        ...
                        style={{
                          background: isSelected
                            ...
                            : 'rgba(30, 41, 59, 0.6)',
                          border:
                            '1px solid ' + (isSelected ? '#3b82f6' : 'rgba(255,255,255,0.08)'),
                ...
                >
                  {videoFormats.length > 0 ? (
                    videoFormats.map((f: any) => (
                      ...
                      >
                        {f.resolution || `${f.height}p`} {f.fps ? `• ${f.fps}fps` : ''}{' '}
                        {f.ext ? `(${f.ext})` : ''} {f.note ? `— ${f.note}` : ''}
                      </option>
                    ))
                  ) : (
              ...
              >
                ...
                <div>
                  ...
                  <div style={{ fontSize: 10, opacity: 0.85 }}>
                    Video parçası atılarak doğrudan 320 kbps MP3 ses dosyası indirilecektir.
                  </div>
                </div>
              </div>
            )}
        ...
        {siteProfile === 'file' && (
          ...
          >
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 2 }}>
                Bu dosya doğrudan sunucudan 8 eşzamanlı parçaya bölünerek maksimum bant genişliğiyle
                indirilecektir.
              </div>
            </div>
          </div>
      ...
      >
        ...
        >
          ...
          <span>
            {starting ? 'Başlatılıyor...' : isAudioMode ? 'Hemen İndir (MP3)' : 'Hemen İndir'}
          </span>
        </button>
