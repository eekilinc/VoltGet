...
export function AppSettingsProvider({ children }: { children: ReactNode }) {
  ...
  function setTheme(th: ThemeMode) {
    setThemeState(th);
    applyTheme(th, accent);
    persist({ theme: th });
  }
  function setAccent(ac: AccentColor) {
    setAccentState(ac);
    applyTheme(theme, ac);
    persist({ accentColor: ac });
  }
  ...
  return (
    <AppSettingsCtx.Provider
      value={{ theme, accent, lang, setTheme, setAccent, setLang, t: k => translate(lang, k) }}
    >
      {children}
    </AppSettingsCtx.Provider>
  );
