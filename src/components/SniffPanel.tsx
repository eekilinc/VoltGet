...
function isHlsMaster(url: string) {
  return (
    ...
    url.includes('.m3u8') ||
    (url.includes('/hls/') && (url.includes('cdnimages') || url.includes('playmix')))
  );
}
...
export default function SniffPanel({
  outDir,
  onStartDownload,
  onDirectDownload,
  onHttpDownload,
  ...
  onDirectDownload: (opts: any) => Promise<any>;
  onHttpDownload: (opts: any) => Promise<any>;
}) {
    ...
    if (scrollToSniffId) {
      ...
      if (item) {
        ...
        if (ref) {
          ...
          ref.style.boxShadow =
            '0 0 0 3px color-mix(in srgb, var(--accent-solid) 50%, transparent)';
  ...
  const domains = Array.from(
    new Set(
      items.map(i => {
        try {
          return new URL(i.pageUrl).hostname.replace('www.', '');
        } catch {
          return 'diğer';
        }
      })
  ...
  async function quickDownload(idx: number, it: Sniff) {
    ...
    try {
      if (generic) {
        ...
        await onDirectDownload({
          url: target,
          outDir,
          pageUrl: it.pageUrl,
          filename: it.filename || undefined,
          title: short(it.pageUrl || target),
        });
      } else {
      ...
      toast.error(String(e?.message || e).slice(0, 600), t('downloadFailed'));
    }
  ...
  async function analyze(idx: number, it: Sniff) {
    ...
    try {
      ...
      if (!r.formats || r.formats.length === 0) {
        toast.warning(t('formatNotFound'));
        return;
      }
      setInfos(s => ({ ...s, [idx]: r }));
      setExpanded(s => ({ ...s, [idx]: true }));
    } catch (e: any) {
      toast.error(String(e?.message || e).slice(0, 700), t('analysisError'));
    }
  ...
  async function downloadWithFormat(it: Sniff, formatId?: string, asAudio?: boolean) {
    ...
    await onStartDownload({
      url: target,
      outDir,
      formatId: asAudio ? undefined : formatId,
      asAudio,
      pageUrl: it.pageUrl,
      title: infos[items.indexOf(it)]?.title || short(target),
    });
  }
    ...
    >
      ...
      {!extConnected && (
        <div
          ...
          style={{
            background:
              'linear-gradient(135deg, rgba(234, 179, 8, 0.12) 0%, rgba(245, 158, 11, 0.05) 100%)',
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 8,
                border:
                  '1px solid ' + (filterDomain === 'all' ? 'var(--accent-solid)' : 'var(--border)'),
                background:
                  filterDomain === 'all'
            ...
            {domains.slice(0, 5).map(d => (
              <button
                ...
                style={{
                  ...
                  borderRadius: 8,
                  border:
                    '1px solid ' + (filterDomain === d ? 'var(--accent-solid)' : 'var(--border)'),
                  background:
                    filterDomain === d
                      ? 'color-mix(in srgb, var(--accent-solid) 16%, var(--panel-2))'
                      : 'var(--panel-2)',
        ...
        {items.length === 0 ? (
          <div className="text-muted" style={{ textAlign: 'center', padding: 36 }}>
            ...
            <div style={{ fontSize: 12, marginTop: 8 }}>
              {t('noMediaCapturedYet')}
              <br />
              <span style={{ fontSize: 11 }}>{t('playVideoOrDownload')}</span>
            </div>
          </div>
        ) : (
          (() => {
            const filtered =
              filterDomain === 'all'
                ...
                : items.filter(i => {
                    try {
                      return new URL(i.pageUrl).hostname.replace('www.', '') === filterDomain;
                    } catch {
                      return false;
                    }
                  });
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {filtered.map(it => {
                  ...
                  return (
                    <div
                      ref={el => {
                        if (el) itemRefs.current[`item-${origIdx}`] = el;
                      }}
                      key={origIdx}
                      style={{
                        background: 'var(--panel-2)',
                    ...
                    >
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                        <span
                          style={{
                            ...
                            background: generic
                              ...
                              : chunk
                                ? '#422006'
                                : 'color-mix(in srgb, var(--accent-solid) 20%, transparent)',
                            color: generic || chunk ? '#fbbf24' : 'var(--text)',
                            padding: '2px 7px',
                            borderRadius: 8,
                          }}
                        >
                          {generic ? 'FILE' : chunk ? 'YT' : '• ' + it.type}
                        </span>
                        <span className="text-muted" style={{ fontSize: 11 }}>
                          {it.time}
                        </span>
                        ...
                        >
                          {short(it.pageUrl || it.url)}
                        </span>
                        <button
                          onClick={() => setExpanded(s => ({ ...s, [origIdx]: !s[origIdx] }))}
                          style={{
                            fontSize: 10,
                        ...
                        >
                          {expanded[origIdx] ? t('hide') : t('detail')}
                        </button>
                      </div>
                      {info ? (
                        <div
                        ...
                        >
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <div className="text-muted" style={{ fontSize: 11 }}>
                              {info.extractor} • {info.formats.length} {t('formatsReady')}
                            </div>
                          </div>
                        </div>
                      ) : (
                      ...
                      {!info ? (
                        <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
                          <button
                            onClick={() => quickDownload(origIdx, it)}
                            disabled={!!downloading[origIdx]}
                            className="brand-gradient"
                            style={{
                              flex: 1,
                              color: '#fff',
                          ...
                          >
                            {downloading[origIdx] ? t('downloadingState') : t('quickDownloadBtn')}
                          </button>
                          {!generic && !isHlsMaster(it.url) && (
                            <button
                              onClick={() => analyze(origIdx, it)}
                              disabled={!!analyzing[origIdx]}
                              style={{
                                flex: 1,
                                background: 'var(--panel)',
                                color: 'var(--text)',
                            ...
                            >
                              {analyzing[origIdx] ? t('analyzingState') : t('selectQualityBtn')}
                            </button>
                          )}
                        </div>
                      ) : (
                        ...
                        >
                          ...
                          >
                            {info.formats
                              .filter((f: any) => !f.isAudioOnly)
                              .slice(0, 12)
                              .map((f: any) => (
                                <button
                                  key={f.id}
                                  onClick={() => downloadWithFormat(it, f.id)}
                                  style={{
                                    background: 'var(--panel-2)',
                                    color: 'var(--text)',
                                    border: '1px solid var(--border)',
                          ...
                          >
                            <button
                              onClick={() => downloadWithFormat(it, undefined, true)}
                              style={{
                                background: '#422006',
                                color: '#fbbf24',
                            ...
                            {info.formats
                              ...
                              .map((f: any) => (
                                ...
                                >
                                  ...
                                  <span className="text-muted" style={{ fontSize: 9 }}>
                                    {f.tbr ? `${Math.round(f.tbr)}kbps` : t('audioStream')}
                                  </span>
                                </button>
                              ))}
                          </div>
