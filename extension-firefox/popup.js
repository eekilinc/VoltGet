...
async function desktopFetch(url, options = {}) {
  ...
  return fetch(url, {
    ...options,
    headers: { ...options.headers, Authorization: 'Bearer ' + pairingToken },
  });
}
document.getElementById('pair').onclick = async () => {
  ...
  if (!/^[a-f0-9]{64}$/.test(token)) {
    status.textContent = 'Geçerli bağlantı anahtarını girin';
    return;
  }
  try {
    ...
    if (!response.ok) throw new Error('Anahtar kabul edilmedi');
    await chrome.storage.local.set({ pairingToken: token });
    document.getElementById('pairingToken').value = '';
    status.textContent = 'Bağlandı ✓';
    document.getElementById('hint').textContent = '✓ VoltGet bağlı';
  } catch {
    status.textContent = 'VoltGet açık olmalı ve anahtar doğru olmalı';
  }
};
let currentTabUrl = '';
...
function short(u) {
  try {
    const p = new URL(u);
    return p.hostname.replace('www.', '') + '/' + p.pathname.slice(1, 30);
  } catch {
    return u.slice(0, 40);
  }
}
...
async function render() {
  ...
  if (filter === 'current' && curDomain) {
    filtered = items.filter(
      i => domain(i.pageUrl) === curDomain || domain(i.tabUrl || '') === curDomain
    );
  }
  ...
  list.innerHTML =
    filtered
      ...
      .map((i, idx) => {
        ...
        return `
    <div class=card>
      <div class=meta><span>${escapeHTML(i.type)} • ${escapeHTML(i.time)}</span><span>${escapeHTML(short(i.pageUrl))}</span></div>
      <div class=url title="${escapeHTML(i.url)}">${escapeHTML(short(i.url))}</div>
      <div class=row>
      ...
      .join('') +
    (filtered.length > 8
      ? `<div style="font-size:10px;opacity:0.6;text-align:center;margin-top:6">+${filtered.length - 8} daha • Tümü'nde gör</div>`
      : '');
  list.querySelectorAll('button').forEach(b => {
    b.onclick = async () => {
      ...
      if (b.dataset.act === 'copy') {
        ...
        try {
          const response = await desktopFetch('http://127.0.0.1:8765/sniff', {
            method: 'POST',
          ...
          b.textContent = 'Bağlantıyı kontrol edin';
          return;
        }
        const toCopy = it.pageUrl?.includes('youtube.com/watch') ? it.pageUrl : it.url;
        await navigator.clipboard.writeText(toCopy);
        b.textContent = 'Gönderildi ✓';
        setTimeout(() => (b.textContent = 'Uygulamaya Gönder'), 1000);
      }
    };
  });
}
...
document.getElementById('clearPage').onclick = async () => {
  ...
  await chrome.storage.local.set({ items: kept });
  render();
};
document.getElementById('open').onclick = async () => {
  try {
    const r = await desktopFetch('http://127.0.0.1:8765/status');
    if (r.ok) {
      document.getElementById('hint').textContent = '✓ Uygulama açık — Yakalayıcı sekmesine bak';
      document.getElementById('hint').style.color = '#86efac';
      return;
    }
  } catch {}
  document.getElementById('hint').textContent =
    '✗ VoltGet kapalı — Lütfen VoltGet uygulamasını başlatın';
  document.getElementById('hint').style.color = '#fca5a5';
};
document.getElementById('autoClear').onchange = async e => {
  await chrome.storage.local.set({ autoClearOnNavigate: e.target.checked });
};
chrome.storage.local
  .get({ autoClearOnNavigate: false })
  .then(v => (document.getElementById('autoClear').checked = v.autoClearOnNavigate));

desktopFetch('http://127.0.0.1:8765/status')
  .then(r =>
    r.ok
      ? ((document.getElementById('hint').textContent =
          '✓ VoltGet açık — yakalananlar otomatik düşer'),
        (document.getElementById('hint').style.color = '#86efac'))
      : null
  )
  .catch(() => {
    document.getElementById('hint').textContent = '○ VoltGet kapalı — Uygulamayı açın';
  });
render();
