...
export default function DownloadPanel({
  ...
  async function analyze(targetUrl?: string) {
    ...
    try {
      ...
      setError(String(e?.message || e).slice(0, 800));
    }
    setLoading(false);
  }
        ...
        <div className="card-premium glow-accent" style={{ borderRadius: 18, padding: 18 }}>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 12, alignItems: 'center' }}>
            ...
            >
              {batchAdding
                ? t('addingState')
                : `🚀 ${extractedBatchUrls.length} ${t('addBatchToQueue')}`}
            </button>
          </div>
        </div>
      )}
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div className="text-muted" style={{ fontSize: 12, marginTop: 4 }}>
                {info.uploader} • {info.extractor} • {Math.floor((info.duration || 0) / 60)}:
                {String((info.duration || 0) % 60).padStart(2, '0')}
              </div>
              <div
                style={{
                  fontSize: 11,
              ...
              >
                {info.formats.length} {t('formatsReady')}
              </div>
            </div>
          </div>
          {!asAudio && (
                ...
                >
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 8,
                      border: 0,
                      background: formatTab === 'video' ? 'var(--accent-solid)' : 'transparent',
                      color: formatTab === 'video' ? '#fff' : 'var(--text)',
                      fontSize: 11,
                      fontWeight: 700,
                      cursor: 'pointer',
                    }}
                  ...
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 8,
                      border: 0,
                      background: formatTab === 'audio' ? '#ca8a04' : 'transparent',
                      color: formatTab === 'audio' ? '#fff' : 'var(--text)',
                      fontSize: 11,
                      fontWeight: 700,
                      cursor: 'pointer',
                    }}
