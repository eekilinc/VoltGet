    ...
    >
      ...
      {isDragOver && (
        <div style={{
          ...
          <div style={{ fontSize: 18, fontWeight: 900, color: '#60a5fa' }}>Bağlantıyı Buraya Bırakın!</div>
          <div style={{ fontSize: 12, color: '#cbd5e1' }}>Otomatik olarak algılanıp analize başlanacak</div>
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 6, background: 'var(--panel-2)', padding: 4, borderRadius: 14, border: '1px solid var(--border)' }}>
        ...
        >
          <span>📦</span>
          <span>Toplu Link / Metin Ayıklayıcı</span>
        ...
        <div className="card-premium glow-accent" style={{ borderRadius: 18, padding: 18 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            ...
            <button
              ...
              title="Panodaki son kopyalanan linki yapıştır"
              style={{
            ...
            >
              <span>📋</span> <span>Panodan Al</span>
            </button>
          </div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <input
              ...
              placeholder="https://youtube.com / tiktok / instagram / x / doğrudan dosya..."
          ...
          <div className="text-muted" style={{ marginTop: 8, fontSize: 11, textAlign: 'center' }}>
            💡 İpucu: Herhangi bir tarayıcıdan linki bu pencerenin içine sürükleyip bırakabilirsiniz!
          </div>
        </div>
      ) : (
        <div className="card-premium glow-accent" style={{ borderRadius: 18, padding: 18 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            <div style={{ fontWeight: 900, fontSize: 14, display: 'flex', alignItems: 'center', gap: 10 }}>
              <span className="brand-gradient" style={{ width: 30, height: 30, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff' }}>📦</span>
              Toplu Bağlantı ve Metin Ayıklayıcı
            </div>
            <span style={{ fontSize: 11, background: 'var(--panel-2)', padding: '4px 8px', borderRadius: 8, border: '1px solid var(--border)' }}>
              {extractedBatchUrls.length} bağlantı bulundu
            </span>
          </div>
          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 8 }}>
            İçinde linkler bulunan karışık bir metin yapıştırın veya her satıra bir link yazın. VoltGet tüm geçerli medyaları otomatik filtreler.
          </div>
          <textarea
            value={batchText}
            onChange={e => setBatchText(e.target.value)}
            placeholder={`Örnek metin veya çoklu linkler:\nhttps://www.youtube.com/watch?v=...\nhttps://example.com/video.mp4\nhttps://example.com/file.zip`}
            rows={6}
            style={{
              width: '100%',
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 12, alignItems: 'center' }}>
            <label style={{ display: 'flex', gap: 6, alignItems: 'center', background: 'var(--panel-2)', border: '1px solid var(--border)', padding: '8px 12px', borderRadius: 10, fontSize: 12, cursor: 'pointer' }}>
              <input type="checkbox" checked={asAudio} onChange={e => setAsAudio(e.target.checked)} /> 🎵 Tümünü MP3 Yap
            </label>
            <button
            ...
            >
              {batchAdding ? 'Ekleniyor...' : `🚀 ${extractedBatchUrls.length} Bağlantıyı Kuyruğa Ekle`}
            </button>
          </div>
        </div>
      )}
        ...
        <div className="card-premium" style={{ borderRadius: 18, padding: 16 }}>
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 11, background: 'var(--panel-2)', display: 'inline-block', padding: '3px 8px', borderRadius: 8, marginTop: 8, border: '1px solid var(--border)' }}>{info.formats.length} format hazır</div>
            </div>
          </div>
          {!asAudio && (
